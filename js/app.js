...
function computeBandGainAdjustmentDb(mhz, antennaType) {
  let ref = 14.0;
  let delta = Math.abs(mhz - ref) / ref;
...
function computeAzimuthPattern(antennaType, reflectors, directors) {
  ...
  for (let deg = 0; deg <= 360; deg += 2) {
    ...
    if (antennaType.startsWith("yagi") || antennaType === "moxon" || antennaType === "bobtail" || antennaType === "half_square") {
    ...
    if (antennaType === "quarter_vertical" || antennaType === "rybakov" || antennaType === "jpole_duct") {
...
function computeElevationPattern(antennaType, seaside, timeOfDay, heightFt, mhz) {
  ...
  for (let deg = 0; deg <= 90; deg += 2) {
    ...
    if (antennaType === "half_dipole" || antennaType === "doublet" || antennaType === "fan_dipole" || antennaType === "ocf_dipole" || antennaType === "edz") {
...
function drawPolarPattern(canvasId, pattern, label) {
...
function drawDonutProjection(canvasId, pattern, label) {
  ...
  for (let i = 0; i < pattern.length; i++) {
    ...
    const y = cy + r * Math.sin(theta) * 0.5;
    if (i === 0) ctx.moveTo(x, y);
    else ctx.lineTo(x, y);
  }
  ctx.stroke();
...
function renderFeedlineSummary(mhz, coaxType, coaxLengthFt, coaxVf) {
...
function applyProfileToUI(profile) {
  ...
  document.getElementById("height_ft").value = profile.height_ft || 35;

  document.getElementById("ladder_type").value = profile.ladder_type || "none";
  document.getElementById("ladder_vf").value = (profile.ladder_vf || 0.92).toFixed(2);
  document.getElementById("ladder_length_ft").value = profile.ladder_length_ft || 50;

  document.getElementById("coax_type").value = profile.coax_type || "lmr400";
  document.getElementById("coax_vf").value = (profile.coax_vf || 0.85).toFixed(2);

  const coaxLen = profile.coax_length_ft || 75;
  ...
  document.getElementById("tx_power_w").value = profile.tx_power_w || 100;
  document.getElementById("radial_count").value = profile.radial_count || 0;
  document.getElementById("elevated_radials").value = profile.elevated_radials || 0;
  document.getElementById("seaside_mode").checked = !!profile.seaside_mode;
  document.getElementById("time_of_day").value = profile.time_of_day || "day";
  document.getElementById("reflector_count").value = profile.reflectors || 0;
  document.getElementById("director_count").value = profile.directors || 0;
}
...
function calculateAndRender() {
  ...
  renderFeedlineSummary(mhz, coaxType, coaxLengthFt, coaxVf);
  ...
  drawPolarPattern("azimuth_plot", az, "Azimuth");
  drawPolarPattern("elevation_plot", el, "Elevation");
  ...
  if (ladderType !== "none") {
    ...
    ladderInfo.push(`<div><strong>Ladder Line:</strong> ${ladderLengthFt.toFixed(1)} ft @ VF ${ladderVf.toFixed(2)}</div>`);
    ladderInfo.push(`<div><strong>Ladder Electrical Length:</strong> ${ladderElecDeg.toFixed(1)}°</div>`);
  }
  const feedlineSummary = document.getElementById("feedline_summary");
  if (feedlineSummary && ladderInfo.length) {
    feedlineSummary.innerHTML += ladderInfo.join("");
  }
}
